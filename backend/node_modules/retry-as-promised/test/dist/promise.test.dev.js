...
describe('Global Promise', function () {
  ...
  it('should reject immediately if max is 1 (using options)', function () {
    ...
    return expect(retry(callback, {
      max: 1,
      backoffBase: 0
    })).to.eventually.be.rejectedWith(this.soRejected).then(function () {
  ...
  it('should reject immediately if max is 1 (using integer)', function () {
    ...
    callback.resolves(this.soResolved);
    callback.onCall(0).rejects(this.soRejected);
    return expect(retry(callback, 1)).to.eventually.be.rejectedWith(this.soRejected).then(function () {
      expect(callback.callCount).to.equal(1);
    });
  });
  it('should reject after all tries if still rejected', function () {
    ...
    return expect(retry(callback, {
      ...
      expect(callback.secondCall.args).to.deep.equal([{
        current: 2
      }]);
      expect(callback.thirdCall.args).to.deep.equal([{
        current: 3
      }]);
  ...
  it('should resolve immediately if resolved on first try', function () {
    ...
    return expect(retry(callback, {
      max: 10,
      backoffBase: 0
    })).to.eventually.equal(this.soResolved).then(function () {
      expect(callback.callCount).to.equal(1);
    });
  });
  ...
  describe('options.timeout', function () {
    ...
    it('should throw if reject on last attempt', function () {
      return expect(retry(function () {
        ...
        backoffBase: 0,
        timeout: 1500
      })).to.eventually.be.rejectedWith(retry.TimeoutError).then(function () {
  ...
  describe('options.match', function () {
    ...
    it('should continue retry while error is equal to match string in array', function () {
      ...
      return expect(retry(callback, {
        max: 15,
        backoffBase: 0,
        match: ['Error: ' + (this.soRejected.message + 1), 'Error: ' + this.soRejected.message]
      })).to.eventually.equal(this.soResolved).then(function () {
        expect(callback.callCount).to.equal(5);
      });
    });
    it('should reject immediately if error is not equal to match string in array', function () {
      ...
      return expect(retry(callback, {
        max: 15,
        backoffBase: 0,
        match: ['Error: ' + (this.soRejected + 1), 'Error: ' + (this.soRejected + 2)]
      })).to.eventually.be.rejectedWith(Error).then(function () {
        expect(callback.callCount).to.equal(1);
      });
    });
  ...
  describe('options.backoff', function () {
    it('should resolve after 5 retries and an eventual delay over 611ms using default backoff', function _callee() {
      var callback, startTime, result, endTime;
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              callback = sinon.stub();
              callback.rejects(this.soRejected);
              callback.onCall(5).resolves(this.soResolved); // When

              startTime = moment();
              _context.next = 6;
              return regeneratorRuntime.awrap(retry(callback, {
                max: 15
              }));
    ...
    it('should resolve after 1 retry and initial delay equal to the backoffBase', function _callee2() {
      var initialDelay, callback, startTime, result, endTime;
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 7:
              ...
              endTime = moment();
              expect(result).to.equal(this.soResolved);
              expect(callback.callCount).to.equal(2); // allow for some overhead
  ...
  describe('options.report', function () {
    it('should receive the error that triggered a retry', function () {
      ...
      return expect(retry(callback, {
        ...
        expect(callback.callCount).to.equal(2); // messages sent to report are:
    ...
    it('should receive the error that exceeded max', function () {
      ...
      return expect(retry(callback, {
        ...
        expect(callback.callCount).to.equal(3); // Trying functionStub #1 at <timestamp>
        // Error: <random number>
        // Retrying functionStub (2)
        // Delaying retry of functionStub by 100
        // Trying functionStub #2 at <timestamp>
